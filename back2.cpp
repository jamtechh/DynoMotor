...
int main(int argc, char* argv[]) {
    ...
    ChVector3d posOffset =  positions[4];            // this is to just move all by a distance from frame to the origin
    
    // Initialize RigidBody objects and store values (starting from index 1)
    bodies[1] = std::make_unique<RigidBody>(sys, file_names[1], true); // motor
    bodies[2] = std::make_unique<RigidBody>(sys, file_names[2]);
    bodies[3] = std::make_unique<RigidBody>(sys, file_names[3]);
    bodies[4] = std::make_unique<RigidBody>(sys, file_names[4], true); // frame
    bodies[5] = std::make_unique<RigidBody>(sys, file_names[5]);bodies[5]->setColor(ChColor(0,1,0));
    bodies[6] = std::make_unique<RigidBody>(sys, file_names[6]);
    bodies[7] = std::make_unique<RigidBody>(sys, file_names[7]);
    bodies[8] = std::make_unique<RigidBody>(sys, file_names[8]);
    bodies[9] = std::make_unique<RigidBody>(sys, file_names[9]);
    bodies[10] = std::make_unique<RigidBody>(sys, file_names[10], true);   // dyno
    for (size_t i = 1; i < file_names.size(); ++i) {
        body_ptrs[i] = bodies[i]->GetBody();
        cogs[i] = bodies[i]->GetCOG();
        body_ptrs[i]->SetPos(positions[i]- posOffset);
    ...
    vis->AddLight(ChVector3d(300.f, 300.f, -300.f), 2000, ChColor(0.1f, 0.1f, 0.1f));
    vis->AddLight(ChVector3d(300.f, -300.f, 300.f), 2000, ChColor(0.1f, 0.1f, 0.1f));
    vis->AddLight(ChVector3d(-300, -400, 0), 2000, ChColor(0.1f, 0.1f, 0.1f));
    vis->AddLight(ChVector3d(-300, 400, 0), 2000, ChColor(0.1f, 0.1f, 0.1f));
