...
int main(int argc, char* argv[]) {
    ...
    // ===========================================================================================================================================================================================
    double t_simulation_STOP = 0.09; //[s]
    ...
    ChElectronicCircuit dyno_Circuit(Netlist_location, t_step_electronic); 
    dyno_Circuit.Initialize(t_step_mechanic);
    dyno_Circuit.InputDefinition(dynoPWLIn, dynoFlowIn);

    high_resolution_clock::time_point start = high_resolution_clock::now();

    double dutyCycle = 10.0;
    double Duty_PWM = dutyCycle / 100; //[s] PWM Duty
    while (t_sim_mechanics <= t_simulation_STOP && vis->Run()) {
        ...
        if(std::abs(fmod(t_sim_mechanics, 0.01)) < 1e-4){
            dutyCycle += 10.0;
            Duty_PWM = dutyCycle / 100;
            std::cout<<"\t\t\t\n dutyCycle =  "<< dutyCycle <<"\n\n\n\n\n";
        }
        if (t_sampling_electronic_counter >= T_ToSample_electronic) {
            Generic_Circuit.Advance(t_step_mechanic);
            dyno_Circuit.Advance(t_step_mechanic);
            auto res1 = Generic_Circuit.GetResult();
            auto res2 = dyno_Circuit.GetResult();
            Rotor_Euler_Vel = RotorBody->GetAngVelLocal(); // Get the effective euler angular velocity 
            dyno_Euler_Vel = dynoBody->GetAngVelLocal(); // Get the effective euler angular velocity 
            
            // double kv_motor = 1600;       // rpm/v >>>>> Checked!!!!!!
            double kv_motor = 2000;       // rpm/v
            double ke_motor = 1/kv_motor; //[V/rpm]
            double kv_dyno = 2000;       // rpm/v
            double ke_dyno = 1/kv_motor; //[V/rpm]
            double Vbackemf = ke_motor * Rotor_Euler_Vel[AngVelAxis];
            double Vbackemf_dyno = ke_dyno * dyno_Euler_Vel[AngVelAxis];
            Imotor = res1[toLowerCase("VmotorVAR")].back();
            Idyno = res2[toLowerCase("VmotorVAR")].back();
            ...
            Generic_Circuit.InputDefinition(PWLIn, FlowIn);
            dynoPWLIn["VmotorVAR"] = -Vbackemf_dyno;
            dyno_Circuit.InputDefinition(dynoPWLIn, dynoFlowIn);
        ...
        double kt_dyno = -50.6; //[Nm/A]
        double sinWave = 0.75 * sin(400 * t_sim_mechanics); 
        double loadTorque = kt_dyno * Idyno * 1e3 * 1e3;
        ...
        OutputMap["alpha"].push_back(loadTorque);
        ...
        std::cout << "dyno I: " <<  Idyno << "\t";
        std::cout << "dyno RPM: " <<  dyno_Euler_Vel[AngVelAxis] << "\t";
        std::cout << "Load Torque: " <<  loadTorque << "\t";
